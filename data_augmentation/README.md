...
As explained above, sometimes evaluating or sampling $p(\theta | Y)$ might be too hard, or we would like a better evaluation when data are partially missing. So we augment the observed data $Y$ with latent variables $Z$. Assuming we can evaluate or sample the augmented posterior $p(\theta | Y, Z)$, we can integrate $Z$ out: $$p(\theta | Y) = \int_{Z} p(\theta | Y, Z)p(Z | Y) dz.$$ We call this the posterior identity. We still need to calculate $p(Z | Y)$ in order to evaluate the posterior identity. We have $$p(Z | Y) = \int_{\Theta} p(Z | Y, \phi)p(\phi | Y)d\phi.$$ That is the predictive identity. Note, we are integrating over $\Theta$, but with respect to $\phi$. If we substitute the predictive identity into the posterior identity, and change the order of integration, we have the following equations: $$p(\theta | Y) = g(\theta) = \int_{\Theta} K(\theta, \phi)g(\phi)d\phi,$$ for $$K(\theta, \phi) = \int_{Z} p(\theta | Z, Y)p(Z, |\phi, Y)dZ.$$ Using successive substitution, we have developed an iterative scheme. We have $$g_{i+1}(\theta) = \int_{\Theta} K(\theta, \phi)g_i(\phi)d\phi.$$ Using Monte Carlo integration, the iterative scheme procedes as follows:

(1) Sample $\theta_{1}^{\ast}$, ..., $\theta_{m}^{\ast}$ from the current approximation to the posterior $p(\theta | Y).$ <br>
(2) Sample $z_1, ..., z_m$ from the current predictive distribution $p(Z | \theta^\ast, Y).$ <br>
(3) Update $p(\theta | Y)$ as the mixture of augmented posteriors, i.e. $p(\theta | Y)$ = $\frac{1}{m}$ $\sum\limits_{j=1}^{m} p(\theta | z_j, Y)$. <br>

To be clear, we repeat these steps many times so $g(\theta)$ converges to $p(\theta | Y)$. We repeat these steps, and eventually, the approximation to the posterior distribution will converge to the real posterior. We demonstrate this process with examples and code below. 
