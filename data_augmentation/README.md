...
a. Sample $\theta^{\ast}_{1}$, ..., $\theta^{\ast}_{m}$ from the current approximation to the posterior $p(\theta | Y)$ $\newline$
b. Sample $z_1, ..., z_m$ from the current predictive distribution $p(Z | \theta^\ast, Y)$ $\newline$
