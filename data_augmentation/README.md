...
(1) Sample $\theta^{\ast}_{1}$, ..., $\theta^{\ast}_{m}$ from the current approximation to the posterior $p(\theta | Y).$ (2) Sample $z_1, ..., z_m$ from the current predictive distribution $p(Z | \theta^\ast, Y).$ (3) Update $p(\theta | Y)$ as the mixture of augmented posteriors, i.e. $p(\theta | Y) = \frac{1}{m} \sum_{j=1}^{m} p(\theta | z_j, Y).$
